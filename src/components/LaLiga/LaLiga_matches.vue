        ...
        <div class="text-center border rounded border-dark bg-dark text-light p-2" v-if="match.utcDate >= fecha && match.utcDate <= fecha2">
          ...
          <div class="d-flex align-items-center flex-row justify-content-center border border-light">
            <div class="flex-fill font-weight-bold w-25">
              <div v-for="(team, index2) in teamsLaLiga2.teams" :key="index2">
                <div v-if="team.id == match.homeTeam.id">
                  <div>
                    <router-link class="text-light text-wrap" :to="{name: 'TeamLaLiga', params:{id: match.homeTeam.id}}">
                      {{match.homeTeam.name}}
                    </router-link>
                  </div>
                </div>
              ...
              <div v-for="(team, index2) in teamsLaLiga2.teams" :key="index2">
                <div v-if="team.id == match.homeTeam.id">
                  <div class="contentLogo">
                    <router-link :to="{name: 'TeamLaLiga', params:{id: team.id}}"><b-img v-bind:src="team.crestUrl" alt="" class="logos"></b-img></router-link>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="flex-fill font-weight-bold w-25">
              <div v-for="(team, index2) in teamsLaLiga2.teams" :key="index2">
                <div v-if="team.id == match.awayTeam.id">
                  <div>
                    <router-link class="text-light text-wrap" :to="{name: 'TeamLaLiga', params:{id: match.awayTeam.id}}">
                      {{match.awayTeam.name}}
                    </router-link>
                  </div>
                </div>
